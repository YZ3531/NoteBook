...
我们先使用express框架写好留言管理的相关接口，并通过postman的测试，然后在某个前端网页中通过ajax来访问，就会出现跨域错误。
...
// 应用级中间件，输出访问日志
app.use(function(req, res, next) {
  console.log(`${Date.now()}:来自${req.connection.remoteAddress} 访问了 ${req.method}-${req.url}。参数是：${req.query}`);
...
1. 启动如上web服务。
2. 使用postman进行测试，这两个接口是可以正常工作的。



第二步，用ajax来请求接口

1. 写一个显示留言板的页面。这个页面在学习ajax时写过的。
...
2.通过vscode中liveserver打开这个页面，就会发现错误了。



![前后端分开布署](node-讲义.assets/1570891970763.png)



如果你把前后端的代码放在同一个服务器就不会出现跨域的问题了。如下

![前后端合并布署](node-讲义.assets/1570892258833.png)
...
![1570940663046](node-讲义.assets/1570940663046.png)
...
app.all('*', function(req, res, next) {
  ...
  res.header('Access-Control-Allow-Credentials', true); //后端能收cookies
  next();
});
...
if(登陆成功){
	// 写入cookie 
	// 后续请求都会带cookie
	res.setHeader('set-cookie', 'name=curry'); //原生的
	
}
})

app.get("/checklogin",(req,res)=>{
	// 取出本次请求中携带的cookie ==> req.headers.cookie
	// 看不是我上次设置的
	console.log( req.cookies )
...
如果是发跨哉的ajax请求需要带上cookie的话，要处理如下：
